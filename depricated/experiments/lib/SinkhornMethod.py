...
class SinkhornMethod:
    ...
    def _new_phi(self, C, p, q, xk):
        return - np.sum(self.lambda_ * p) - np.sum(self.my * q) - self.gamma * np.sum(xk * np.exp(-(C + np.tile(self.lambda_, (self.n, 1)).T + np.tile(self.my, (self.n, 1)) + self.gamma) / self.gamma))
